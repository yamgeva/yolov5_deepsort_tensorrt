...
def update(bboxes, image):
    ...
    if len(bboxes) > 0:
        ...
        outputs = deepsort.update(xywhs, lbls, confss, image)

        for value in list(outputs):
            x1, y1, x2, y2, track_label, track_id = value
            bboxes2draw.append((int(x1), int(y1), int(x2), int(y2), track_label, int(track_id)))
        pass
    pass
